...
import {
  extensionService,
  SessaoExtensao,
  SessaoExtensaoDetail,
} from "@/features/analysis/api/extensionService";
...
export function ExtensionLinkerModal({
  isOpen,
  onClose,
  jornadaId,
  etapaId,
  ...
  processoId,
  onSuccess,
}: ExtensionLinkerModalProps) {
  const [sessions, setSessions] = useState<SessaoExtensao[]>([]);
  const [selectedSession, setSelectedSession] =
    useState<SessaoExtensaoDetail | null>(null);
          ...
          {!selectedSession ? (
            <div className="space-y-6 h-full flex flex-col">
              <div className="bg-blue-50/80 p-4 rounded-2xl border border-blue-100 flex gap-3">
                ...
                <p className="text-xs text-blue-700 font-bold leading-relaxed uppercase tracking-tight">
                  Selecione uma sessão gravada pela extensão para extrair o
                  tempo real desta etapa no fluxo.
                </p>
              </div>
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-2">
                              <Badge
                                variant="secondary"
                                className="text-xs font-black uppercase tracking-tighter px-2 h-4"
                              >
                                {format(
            ...
            <div className="space-y-6 h-full flex flex-col animate-in slide-in-from-right-4 duration-300">
              <Button
                ...
                className="text-xs font-black text-primary p-0 h-auto uppercase tracking-[0.2em] w-fit"
              >
                ...
                <div className="relative z-10 flex items-center justify-between">
                  <div>
                    <h4 className="text-sm font-black uppercase tracking-widest mb-1">
                      Click Stream da Sessão
                    </h4>
                    <p className="text-xs text-slate-400 font-bold uppercase tracking-widest italic opacity-70">
                      Selecione o marco de início e o marco de fim
                    </p>
                  </div>
                  {startPoint && endPoint && (
                    <Badge className="bg-primary/20 text-primary border-primary/30 animate-pulse font-black text-xs px-3">
                      INTERVALO DEFINIDO
                    </Badge>
                  )}
                </div>
                ...
                <div className="space-y-2">
                  {allInteractions.map((inter, i) => {
                    ...
                    const isInRange =
                      startPoint &&
                      endPoint &&
                      inter.timestamp_evento >=
                        Math.min(startPoint, endPoint) &&
                        ...
                        <div className="flex items-start justify-between gap-4">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              ...
                              <p
                                className={cn(
                                  "text-xs font-bold mt-1 line-clamp-1",
                                  isStart || isEnd
                                    ? "text-white/70"
                                    : "text-slate-400",
                                )}
                          ...
                          <div className="shrink-0">
                            ...
                            >
                              {format(
                                new Date(inter.timestamp_evento),
                                "HH:mm:ss.SSS",
                              )}
                            </p>
                          </div>
