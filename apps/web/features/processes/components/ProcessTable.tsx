...
import { Edit2, ArrowRight, Landmark, Activity, SearchX, Trash2, Copy, Check } from "lucide-react";
import { useAuth } from "@/features/auth/context/AuthContext";
import { useDeleteProcessMutation } from "../api/useProcessQueries";
...
export function ProcessTable({
  ...
  const handleDelete = (id: number, nome: string) => {
    if (window.confirm(`Tem certeza que deseja excluir o processo "${nome}"? Esta ação não pode ser desfeita.`)) {
    ...
    <div className="w-full">
      {loading ? (
        ...
        <div className="py-20 text-center m-4">
          <p className="text-destructive font-black italic uppercase tracking-tighter">
        ...
        <Table>
          <TableHeader className="bg-slate-50/50">
            <TableRow className="hover:bg-transparent border-none">
              <TableHead className="px-8 h-14 text-xs font-black text-slate-400 uppercase tracking-widest text-center w-20">
                ID
              </TableHead>
              ...
              {processos.map((p, idx) => (
                ...
                >
                  ...
                  <TableCell className="px-8 py-5">
                    <Link
                      href={`/processos/${p.id}`}
                      className="text-sm font-bold text-slate-900 hover:text-primary transition-colors cursor-pointer block"
                    >
                      {p.nome}
                    </Link>
                    <div className="text-xs font-medium text-slate-400 mt-0.5 line-clamp-1 max-w-md">
                      {p.descricao ||
                        "Sem descrição disponível para este processo"}
                    </div>
                  </TableCell>
                  <TableCell className="px-8 py-5">
                  ...
                  <TableCell className="px-8 py-5">
                    <div className="flex justify-center">
                      <Badge
                        ...
                        className={cn(
                          "px-3 py-1 rounded-full text-xs font-black uppercase tracking-tighter gap-1.5",
                          p.status === "Finalizado" &&
                  ...
                  <TableCell className="pl-8 pr-4 py-5 text-center">
