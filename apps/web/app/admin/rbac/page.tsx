...
const roleConfig = {
  admin: { label: "Administrador", color: "destructive", bg: "bg-red-500/10", icon: ShieldCheck },
  researcher: { label: "Pesquisador", color: "default", bg: "bg-blue-500/10", icon: Shield },
  supervisor: { label: "Supervisor", color: "outline", bg: "bg-amber-500/10", icon: UserIcon },
  visitor: { label: "Visitante", color: "secondary", bg: "bg-slate-500/10", icon: ShieldAlert },
};
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-1">
          ...
          <p className="text-slate-500 font-medium max-w-xl">
            Painel administrativo para controle de permissões e autorização de e-mails via Google Auth.
          </p>
        </div>
          ...
          <Table>
            <TableHeader className="bg-slate-50/50">
              <TableRow className="hover:bg-transparent">
                <TableHead className="px-8 h-14 text-[10px] font-black text-slate-400 uppercase tracking-widest">Usuário</TableHead>
                <TableHead className="px-8 h-14 text-[10px] font-black text-slate-400 uppercase tracking-widest">Nível de Acesso</TableHead>
                <TableHead className="px-8 h-14 text-[10px] font-black text-slate-400 uppercase tracking-widest">Cadastro</TableHead>
                ...
                {isLoading ? (
                  Array.from({ length: 3 }).map((_, i) => (
                    <TableRow key={i} className="animate-pulse">
                      <TableCell colSpan={4} className="h-24 px-8 bg-slate-50/20"></TableCell>
                    </TableRow>
                  ))
                    ...
                    >
                      <TableCell className="px-8 py-6">
                        <div className="flex items-center gap-4">
                          ...
                          <div className="space-y-0.5">
                            <p className="font-bold text-slate-900 tracking-tight">{item.email}</p>
                            <Badge variant="outline" className="text-[9px] h-4 font-black uppercase tracking-widest border-slate-200 text-slate-400">
                              Google OAuth 2.0
                            </Badge>
                          </div>
                        </div>
                      ...
                      <TableCell className="px-8 py-6">
                        ...
                        >
                          ...
                          <SelectContent className="rounded-xl border-slate-100 shadow-xl">
                            <SelectItem value="admin" className="text-xs font-bold text-red-600">Administrador</SelectItem>
                            <SelectItem value="researcher" className="text-xs font-bold text-blue-600">Pesquisador</SelectItem>
                            <SelectItem value="supervisor" className="text-xs font-bold text-amber-600">Supervisor</SelectItem>
                            <SelectItem value="visitor" className="text-xs font-bold text-slate-500">Visitante</SelectItem>
                          </SelectContent>
                        </Select>
                      </TableCell>
                      <TableCell className="px-8 py-6">
