...
export default function ProcessoDetailPage({
  ...
  const {
    data: processo,
    isLoading: loadingProcesso,
    error: errorProcesso,
  } = useProcessDetail(processId);
  const { data: analysis, isLoading: loadingAnalysis } =
    useProcessAnalysis(processId);
    ...
    return (
      <div className="p-8 text-center max-w-md mx-auto mt-20">
        ...
        <p className="text-slate-500 mb-8 font-medium">
          O link pode estar quebrado ou você não tem permissão para acessar este
          recurso.
        </p>
    ...
    <div className="space-y-8 pb-10 max-w-7xl mx-auto py-6">
      ...
      <section className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-5">
          ...
          <div>
            <div className="flex items-center gap-2 text-primary font-black text-xs uppercase tracking-[0.2em] mb-1">
            ...
            <CardContent className="p-8 space-y-6">
              <div className="space-y-2">
                <p className="text-sm text-slate-600 leading-relaxed font-medium">
                  {processo.descricao ||
                    "Nenhuma descrição fornecida para este processo."}
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <Badge
                    variant="secondary"
                    className="font-black h-7 uppercase tracking-tighter px-3"
                  >
                ...
                <div className="space-y-1.5">
                  ...
                  <Badge
                    variant="outline"
                    className="font-black h-7 uppercase tracking-tighter px-3 border-slate-200 text-slate-500"
                  >
            ...
            <div className="relative z-10">
              ...
              <p className="text-xs text-blue-100 font-medium leading-relaxed mt-6 opacity-90">
                ...
                <span className="font-black text-white">
                  {processo.etapas.length} interações
                </span>{" "}
                distintas do cidadão para ser concluído.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-8">
          <Tabs defaultValue="analise" className="w-full">
            <TabsList className="mb-8 p-1 bg-slate-100 rounded-2xl w-fit">
              ...
              <TabsTrigger value="capturas" className="rounded-xl px-8 font-black uppercase tracking-widest text-xs data-[state=active]:bg-white data-[state=active]:shadow-sm gap-2">
            ...
            <TabsContent value="analise" className="space-y-8 mt-0 border-none p-0 outline-none">
              <Card className="rounded-[2.5rem] border-slate-100 shadow-xl shadow-slate-200/40 overflow-hidden">
                <CardHeader className="px-8 pt-8 flex flex-row items-center justify-between border-b border-slate-50 bg-slate-50/20">
                  <div className="flex items-center gap-3">
                    ...
                    <CardTitle className="text-xl font-black tracking-tighter uppercase">
                      Análise de Sludge por Etapa
                    </CardTitle>
                  </div>
                  <Badge
                    variant="secondary"
                    className="bg-blue-100 text-primary border-none font-black text-xs px-3 py-1 uppercase tracking-widest"
                  >
                    Live Analysis
                  </Badge>
                </CardHeader>
                <CardContent className="p-8">
                  ...
                  <div className="mt-8 flex items-center justify-between text-xs text-slate-400 font-black uppercase tracking-[0.2em]">
              ...
              <Card className="rounded-[2.5rem] border-slate-100 shadow-xl shadow-slate-200/40 overflow-hidden">
                <CardHeader className="px-8 pt-8 flex flex-row items-center justify-between border-b border-slate-50">
                  ...
                  {canEdit && (
                    <Button
                      size="sm"
                      className="rounded-xl font-black text-xs uppercase tracking-widest gap-2"
                      onClick={() => {
                  ...
                  <Table>
                    <TableHeader className="bg-slate-50/50 hover:bg-transparent">
                      <TableRow className="border-none hover:bg-transparent">
                        <TableHead className="px-8 h-14 text-xs font-black text-slate-400 uppercase tracking-widest w-24 text-center">
                          Ordem
                        </TableHead>
                        <TableHead className="px-8 h-14 text-xs font-black text-slate-400 uppercase tracking-widest">
                          Comportamento
                        </TableHead>
                        <TableHead className="px-8 h-14 text-xs font-black text-slate-400 uppercase tracking-widest w-28 text-center">
                          Planejado
                        </TableHead>
                        <TableHead className="px-8 h-14 text-xs font-black text-slate-400 uppercase tracking-widest w-32 text-center">
                          Observado
                        </TableHead>
                        <TableHead className="px-8 h-14 text-xs font-black text-slate-400 uppercase tracking-widest w-20 text-center">
                          Obrig.
                        </TableHead>
                        {canEdit && (
                          <TableHead className="px-8 h-14 text-xs font-black text-slate-400 uppercase tracking-widest text-center">
                            Ações
                          </TableHead>
                        )}
                      </TableRow>
                    ...
                    <TableBody>
                      <AnimatePresence mode="popLayout">
                        {processo.etapas.length > 0 ? (
                          processo.etapas.map((e, idx) => {
                            const analysisStep = analysis?.steps?.find(
                              (s) => s.etapa_id === e.id,
                            );
                            const isCritical =
                              analysisStep && (analysisStep.prioridade || 0) >= 3;
                            ...
                            return (
                              ...
                              >
                                <TableCell className="px-8 py-5 text-xs font-black text-slate-300 text-center">
                                  #0{e.ordem}
                                </TableCell>
                                <TableCell className="px-8 py-5">
                                  <div className="flex flex-col gap-1">
                                    ...
                                    {analysisStep?.recomendacao && isCritical && (
                                      <Badge
                                        variant="destructive"
                                        className="w-fit text-xs h-5 font-black uppercase tracking-widest gap-1 py-0 px-2 rounded-full"
                                      >
                                        <Info size={10} />
                                ...
                                <TableCell className="px-8 py-5 text-center">
                                  <Badge
                                    variant="outline"
                                    className="h-7 rounded-xl border-blue-100 bg-blue-50/50 text-primary font-black text-xs tabular-nums px-3"
                                  >
                                    {e.duracao_media_observada
                                      ? `${e.duracao_media_observada.split(":")[1]}m`
                                      : "Análise..."}
                                  </Badge>
                                </TableCell>
                                <TableCell className="px-8 py-5">
                                  <div
                                    className={cn(
                                      ...
                                      e.e_obrigatorio
                                        ...
                                        : "bg-slate-100 ring-slate-100/30",
                                    )}
                                  />
                                </TableCell>
                                {canEdit && (
                              ...
                              <div className="flex flex-col items-center justify-center space-y-6">
                                ...
                                <div className="space-y-1">
                                  ...
                                  <p className="text-slate-400 text-xs font-medium">
                                    Comece adicionando a primeira etapa deste fluxo.
                                  </p>
                                </div>
                                {canEdit && (
                                  <Button
            ...
            <TabsContent value="capturas" className="mt-0 border-none p-0 outline-none">
              <CapturedSessions processId={processId} />
            </TabsContent>
          </Tabs>
        </div>
