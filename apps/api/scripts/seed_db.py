...
def protocol_unique(db: Session) -> str:
	...
	while True:
		...
		exists = db.scalar(
			select(func.count())
			.select_from(JornadaObservada)
			.where(JornadaObservada.protocolo == proto)
		)
		if not exists:
			return proto
...
def seed_etapas(db: Session, processo: Processo, n_etapas: int) -> list[Etapa]:
	...
	if not categorias or not tipos:
		raise RuntimeError(
			"Categoria/TipoComportamento vazios. Rode o SQL de dados iniciais primeiro."
		)
...
def seed_criterios_por_etapa(db: Session, etapa: Etapa):
	...
	if not templates:
		raise RuntimeError(
			"CriterioTemplate vazio. Rode o SQL de dados iniciais primeiro."
		)
	...
	impactos = [
		CriterioImpactoEnum.CARGA_COGNITIVA,
		CriterioImpactoEnum.EMOCAO,
		CriterioImpactoEnum.CONSEQUENCIA,
	]
...
def seed_escalas(db: Session):
	...
	for t in templates:
		# Cria uma escala genérica para o critério
		e = EscalaAvaliacao(
			criterio_template_id=t.id,
			pergunta=f"Considerando o ponto de vista da {t.nome}, como você avalia esta etapa?",
			texto_nota_1="Muito difícil/barreira crítica",
			texto_nota_2="Dificuldade considerável",
			texto_nota_3="Neutro/aceitável",
			texto_nota_4="Fácil/bom",
			texto_nota_5="Excelente/sem barreiras",
		)
		db.add(e)

		# Opcionalmente cria escalas específicas para alguns tipos de comportamento
		for tc in tipos[::5]:  # pega um a cada 5 tipos para variar
			e_spec = EscalaAvaliacao(
				criterio_template_id=t.id,
				tipo_comportamento_id=tc.id,
				pergunta=f"Para esta ação de {tc.nome}, como foi a {t.nome}?",
				texto_nota_1="Totalmente inadequado",
				texto_nota_5="Perfeito para o contexto",
			)
			db.add(e_spec)
...
def seed_jornadas(
	db: Session, processo: Processo, observadores: list[Observador], n: int
) -> list[JornadaObservada]:
...
def seed_avaliacoes_e_tempos(
	db: Session, etapas: list[Etapa], jornadas: list[JornadaObservada]
):
	...
	for jo in jornadas:
		for etapa in etapas:
			...
			# barreiras
			barreiras = db.scalars(
				select(CriterioBarreira).where(CriterioBarreira.etapa_id == etapa.id)
			).all()
			for cb in barreiras:
			...
			# impactos
			impactos = db.scalars(
				select(CriterioImpacto).where(CriterioImpacto.etapa_id == etapa.id)
			).all()
			for ci in impactos:
				if random() < 0.75:
					ai = AvaliacaoImpacto(
...
def seed_resultados(db: Session, processo: Processo, etapas: list[Etapa]):
	...
	for etapa in etapas:
		...
		indice = (
			round((mb or 0) * (mi or 0), 2)
			if (mb is not None or mi is not None)
			else None
		)
...
def main():
	...
	try:
		...
		print(f"✅ {N_OBSERVADORES} observadores criados.")

		seed_escalas(db)
		print("✅ Escalas de avaliação padrão criadas.")

		processos = seed_processos(db, N_PROCESSOS)
		print(f"✅ {N_PROCESSOS} processos criados.")
		...
		for p in processos:
			...
			jornadas = seed_jornadas(
				db, p, observadores, randint(*JORNADAS_POR_PROCESSO)
			)
