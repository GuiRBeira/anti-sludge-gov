...
from app.features.analysis import templates_schemas as schemas
...
async def create_grupo_analise(
	obj_in: schemas.GrupoAnaliseCreate, db: AsyncSession = Depends(get_db)
):
...
async def create_criterio_template(
	obj_in: schemas.CriterioTemplateCreate, db: AsyncSession = Depends(get_db)
):
...
async def list_criterios_template(
	skip: int = 0, limit: int = 100, db: AsyncSession = Depends(get_db)
):
...
async def create_tipo_criterio(
	obj_in: schemas.TipoCriterioCreate, db: AsyncSession = Depends(get_db)
):
...
async def create_escala_avaliacao(
	obj_in: schemas.EscalaAvaliacaoCreate, db: AsyncSession = Depends(get_db)
):
...
async def list_escalas_avaliacao(
	skip: int = 0, limit: int = 100, db: AsyncSession = Depends(get_db)
):
