...
if TYPE_CHECKING:
	from app.features.analysis.models import AvaliacaoBarreira, AvaliacaoImpacto
...
class JornadaObservada(Base):
	...
	observador_id: Mapped[int | None] = mapped_column(
	...
	tempos_etapas: Mapped[list[TempoEtapa]] = relationship(
		back_populates="jornada", cascade="all, delete-orphan", passive_deletes=True
	)
	avaliacoes_barreiras: Mapped[list[AvaliacaoBarreira]] = relationship(
		"AvaliacaoBarreira", back_populates="jornada", passive_deletes=True
	)
	avaliacoes_impactos: Mapped[list[AvaliacaoImpacto]] = relationship(
		"AvaliacaoImpacto", back_populates="jornada", passive_deletes=True
	)
...
class TempoEtapa(Base):
	...
	jornada_observada_id: Mapped[int] = mapped_column(
		ForeignKey("jornada_observada.id", ondelete="CASCADE"), nullable=False
	)
