...
function finalizeCurrentPage() {
  ...
  if (!page.leftAt) {
    page.leftAt = isoNow();
    page.durationMs = new Date(page.leftAt) - new Date(page.enteredAt);
...
async function activate() {
  ...
  chrome.action.setIcon({ path: { "48": "icons/icon-on-48.png" } });
...
async function deactivate() {
  ...
  if (session) {
    ...
    session.totalDurationMs = new Date(session.endTime) - new Date(session.startTime);
  ...
  chrome.action.setIcon({ path: { "48": "icons/icon-off-48.png" } });
  chrome.action.setBadgeText({ text: "" });
}

async function downloadSession(sessionData) {
